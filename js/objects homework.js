...
const personalMovieDB = {
  ...
  showMyDB: function () {
    if (personalMovieDB.privat == false) {
      console.log("privat is false", personalMovieDB);
    } else {
  ...
  writeYourGenres: function () {
    for (let i = 1; i <= 3; i++) {
      let perMovie = prompt(`Ваш любимый жанр под номером ${i}`);

      if (perMovie == null || perMovie === "") {
        console.log("Введено некорректное значение");
        i--;
      } else {
        personalMovieDB.genres[i - 1] = perMovie;
      }
    }
